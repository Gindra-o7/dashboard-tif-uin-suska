...
const Step4: FC<Step4Props> = ({ activeStep }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["seminar-kp-step4"] });
    },
  ...
  useEffect(() => {
    if (data?.data) {
      const step4Docs = data.data.dokumen_seminar_kp.step4 || [];
      const step4Accessible = data.data.steps_info.step4_accessible;
      ...
      const initialDocs = DOCUMENTS.map((title) => {
        ...
        return {
          ...
          status:
            ...
            (step4Accessible ? "default" : "Terkirim"),
  ...
  const status: StepStatus = useMemo(() => {
    ...
    // Jika step4_accessible true dan belum ada dokumen yang dikirim
    if (step4Accessible && step4Docs.length === 0) {
      return "belum";
    }

    // Jika ada dokumen yang ditolak
    if (step4Docs.some((doc: any) => doc.status === "Ditolak")) {
      return "ditolak";
    }

    // Filter dokumen wajib
    const mandatoryDocs = step4Docs.filter((doc: any) =>
