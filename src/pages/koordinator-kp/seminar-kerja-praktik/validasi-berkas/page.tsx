...
const KoordinatorValidasiBerkasPage: FC = () => {
  ...
  const [isValidatedStudentModal, setIsValidatedStudentModal] = useState<boolean>(false);
  ...
  const { data, isLoading, isError, error } = useQuery<ApiResponse>({
    ...
    queryFn: () => APISeminarKP.getDataMahasiswa(selectedTahunAjaranId ?? undefined),
    enabled: selectedTahunAjaranId !== null,
  });

  const { data: detailData, isLoading: isDetailLoading } = useQuery<MahasiswaDetailResponse>({
  ...
  useEffect(() => {
    if (tahunAjaranData && tahunAjaranData.length > 0 && selectedTahunAjaranId === null) {
  ...
  useEffect(() => {
    if (data?.data?.tahun_ajaran?.id && data.data.tahun_ajaran.id !== selectedTahunAjaranId && tahunAjaranData?.some((tahun) => tahun.id === data.data.tahun_ajaran.id)) {
      setSelectedTahunAjaranId(data.data.tahun_ajaran.id);
    }
  }, [data, tahunAjaranData, selectedTahunAjaranId]);

  const { students, totalMahasiswa, validasiDitolak, validasiSelesai } = useMemo(() => {
  ...
  const detailedStudent: Student | null = useMemo(() => {
    ...
    const pembimbingInstansi = pendaftaran?.instansi?.pembimbing_instansi?.[0]?.nama || "Tidak tersedia";

    // Define nilaiInstansi here, adjust the logic as needed
    const nilaiInstansi = (detailData.data.nilai && detailData.data.nilai.length > 0 ? detailData.data.nilai[0]?.nilai_instansi : undefined) || "Tidak tersedia";
  ...
  const renderModal = () => {
    ...
    const modalStage = isValidatedStudentModal ? activeTab : detailedStudent.stage;
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} className="bg-white dark:bg-gray-800 border-none shadow-md rounded-lg p-4 animate-pulse">
  ...
  if (isTahunAjaranError) {
    toast.error(`Gagal mengambil daftar tahun ajaran: ${(tahunAjaranError as Error).message}`, {
      duration: 3000,
    });
    return (
      <DashboardLayout>
        <div className="text-center text-gray-600 dark:text-gray-300 py-10">Gagal memuat daftar tahun ajaran. Silakan coba lagi nanti.</div>
      </DashboardLayout>
    );
  }
  ...
  if (isError) {
    ...
    return (
      <DashboardLayout>
        <div className="text-center text-gray-600 dark:text-gray-300 py-10">Gagal memuat data mahasiswa. Silakan coba lagi nanti.</div>
      </DashboardLayout>
    );
  }
      ...
      <div className="transition-colors duration-300">
        <div className="space-y-4">
          ...
          {isLoading || isTahunAjaranLoading ? (
            ...
            <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-3" variants={container} initial="hidden" animate="show">
              <motion.div variants={item}>
                <Card className="overflow-hidden border-none shadow-md bg-gradient-to-br from-blue-100 to-blue-50 dark:from-blue-950 dark:to-gray-900">
                  <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
                    <CardTitle className="text-sm font-medium text-blue-800 dark:text-blue-300">Permohonan Validasi</CardTitle>
                    <motion.div whileHover={{ rotate: 15 }} className="bg-blue-200 p-1.5 rounded-full dark:bg-blue-800">
                      <Users className="h-3.5 w-3.5 text-blue-600 dark:text-blue-300" />
                    </motion.div>
                  </CardHeader>
                  <CardContent className="px-3 pb-3">
                    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", delay: 0.1 }} className="text-2xl font-bold text-blue-800 dark:text-white">
                      {totalMahasiswa ?? 0}
                    </motion.div>
                    <p className="text-xs text-blue-600 dark:text-blue-300 mt-0.5">Mahasiswa</p>
              ...
              <motion.div variants={item}>
                <Card className="overflow-hidden border-none shadow-md bg-gradient-to-br from-red-100 to-red-50 dark:from-red-950 dark:to-gray-900">
                  <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
                    <CardTitle className="text-sm font-medium text-red-800 dark:text-red-300">Validasi Ditolak</CardTitle>
                    <motion.div whileHover={{ rotate: 15 }} className="bg-red-200 p-1.5 rounded-full dark:bg-red-800">
                      <AlertCircle className="h-3.5 w-3.5 text-red-600 dark:text-red-300" />
                    </motion.div>
                  </CardHeader>
                  <CardContent className="px-3 pb-3">
                    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", delay: 0.1 }} className="text-2xl font-bold text-red-800 dark:text-white">
                      {validasiDitolak ?? 0}
                    </motion.div>
                    <p className="text-xs text-red-600 dark:text-red-300 mt-0.5">Mahasiswa</p>
                    <div className="h-1.5 w-full bg-red-100 dark:bg-red-900 rounded-full mt-2">
                      <motion.div
                        initial={{ width: "0%" }}
                        animate={{
                          width: totalMahasiswa ? `${(validasiDitolak / totalMahasiswa) * 100}%` : "0%",
                        }}
                        transition={{ duration: 1.5, ease: "easeOut" }}
              ...
              <motion.div variants={item}>
                <Card className="overflow-hidden border-none shadow-md bg-gradient-to-br from-green-100 to-green-50 dark:from-green-950 dark:to-gray-900">
                  <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
                    <CardTitle className="text-sm font-medium text-green-800 dark:text-green-300">Validasi Selesai</CardTitle>
                    <motion.div whileHover={{ rotate: 15 }} className="bg-green-200 p-1.5 rounded-full dark:bg-green-800">
                      <CheckCircle className="h-3.5 w-3.5 text-green-600 dark:text-green-300" />
                    </motion.div>
                  </CardHeader>
                  <CardContent className="px-3 pb-3">
                    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", delay: 0.1 }} className="text-2xl font-bold text-green-800 dark:text-white">
                      {validasiSelesai ?? 0}
                    </motion.div>
                    <p className="text-xs text-green-600 dark:text-green-300 mt-0.5">Mahasiswa</p>
                    <div className="h-1.5 w-full bg-green-100 dark:bg-green-900 rounded-full mt-2">
                      <motion.div
                        initial={{ width: "0%" }}
                        animate={{
                          width: totalMahasiswa ? `${(validasiSelesai / totalMahasiswa) * 100}%` : "0%",
                        }}
                        transition={{ duration: 1.5, ease: "easeOut" }}
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <Tabs defaultValue="pendaftaran" onValueChange={(value) => setActiveTab(value as Stage)} className="w-full">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 w-full">
                <TabsList className="dark:bg-gray-700">
                  <TabsTrigger value="pendaftaran" className="dark:data-[state=active]:bg-gray-800">
                    Pendaftaran
                  </TabsTrigger>
                  <TabsTrigger value="idSurat" className="dark:data-[state=active]:bg-gray-800">
                    ID Surat Undangan
                  </TabsTrigger>
                  <TabsTrigger value="suratUndangan" className="dark:data-[state=active]:bg-gray-800">
                    Surat Undangan
                  </TabsTrigger>
                  <TabsTrigger value="pascaSeminar" className="dark:data-[state=active]:bg-gray-800">
                    Pasca Seminar
                  </TabsTrigger>
                </TabsList>
              ...
              {["pendaftaran", "idSurat", "suratUndangan", "pascaSeminar"].map((tab) => (
                <TabsContent key={tab} value={tab} className="mt-4">
                  {isLoading || isTahunAjaranLoading ? (
                    <SkeletonTable />
                  ) : (
                    <StudentTable students={filteredStudents} validatedStudents={validatedStudents} onViewDetail={handleOpenDialog} onViewDetailRiwayat={handleOpenDialogRiwayat} activeTab={activeTab} />
                  )}
                </TabsContent>
              ))}
            </Tabs>
...
const StudentTable: FC<{
  ...
  activeTab: Stage;
}> = ({ students = [], validatedStudents = [], onViewDetail, onViewDetailRiwayat, activeTab }) => {
  return (
    <div className="space-y-6">
      {/* Main Table */}
        ...
        <Table>
          <TableHeader className="bg-gray-200 dark:bg-gray-700">
            <TableRow className="hover:bg-gray-300 dark:hover:bg-gray-600">
              <TableHead className="text-center max-w-4 font-semibold dark:text-gray-200">No.</TableHead>
              <TableHead className="text-center font-semibold dark:text-gray-200">Nama Mahasiswa</TableHead>
              <TableHead className="text-center font-semibold dark:text-gray-200">NIM</TableHead>
              <TableHead className="text-center font-semibold dark:text-gray-200">Tahap</TableHead>
              <TableHead className="text-center font-semibold dark:text-gray-200">Waktu Pengajuan</TableHead>
              <TableHead className="text-center font-semibold dark:text-gray-200">Aksi</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {students.length === 0 ? (
              ...
              students.map((student, index) => (
                <TableRow key={student.id} className="dark:border-gray-700 dark:hover:bg-gray-700">
                  <TableCell className="text-center dark:text-gray-300 text-xs font-semibold">{index + 1}.</TableCell>
                  <TableCell className="dark:text-gray-300 text-xs text-center">{student.name}</TableCell>
                  <TableCell className="font-medium text-center dark:text-gray-300 text-xs">{student.nim}</TableCell>
                  <TableCell className="text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${stageBadgeConfig[student.stage].bgColor} ${stageBadgeConfig[student.stage].textColor} ${stageBadgeConfig[student.stage].darkBgColor} ${
                  ...
                  <TableCell className="text-center text-xs text-muted-foreground dark:text-gray-400">{student.timeAgo}</TableCell>
                  <TableCell className="text-center">
                    <Button
                      variant="default"
                      size="sm"
      ...
      {validatedStudents.length > 0 && (
        <div className="shadow-none rounded-none dark:bg-gray-950 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-2">Tervalidasi</h2>
          <Table>
